...
responseBinAnalysis <- function(raw_data, columnToStudy, classificationTargetColumnName, positiveEventResponse="yes", negativeEventResponse="no", no.Of.Bins=10,
                                bin_breaks) {
    if (!is.factor(raw_data[,columnToStudy]) && missing(bin_breaks)) {
        bin_breaks = c(seq(min(raw_data[,columnToStudy]),max(raw_data[,columnToStudy]), length.out=no.Of.Bins))
    }
    if (is.numeric(raw_data[,columnToStudy])) { # The column to study is a numeric/continous, need to convert to bin
        raw_data$binning.tmp <- as.factor(cut(raw_data[,columnToStudy], breaks = bin_breaks))
    } else if (is.factor(raw_data[,columnToStudy])) { # It is already a factor, if bin_breaks not provide use default factor as bins
        if (missing(bin_breaks)) { # If factor and no bin breaks specified, then use levels as bins
            raw_data$binning.tmp <- as.factor(raw_data[,columnToStudy])
        } else { # If factor and new bins specified then force to use the bin breaks sent as param
            raw_data$binning.tmp <- as.factor(cut(raw_data[,columnToStudy], breaks = bin_breaks))
        }
    } else {
        stop('The variable to study shuld be numerical or categorical.')
    }
    
    if (is.character(raw_data[,classificationTargetColumnName]) || is.factor(raw_data[,classificationTargetColumnName])) { # The target column need to be converted to 0,1
        raw_data[,classificationTargetColumnName] <- ifelse(raw_data[,classificationTargetColumnName] == positiveEventResponse, 1, 0)
    }
    
    # Check the numeric value of response rate in each bucket
    aggregate_formula <- as.formula(paste(classificationTargetColumnName,"~", "binning.tmp"))
    
    aggregatedStudyColumn <- merge(aggregate(aggregate_formula, raw_data, mean),aggregate(aggregate_formula, raw_data, sum),by = "binning.tmp") 
    
    # Adding total_population
    count <- data.frame(table(raw_data$binning.tmp))
    colnames(count) <- c("binning.tmp","Frequency")
    #count <- count[,-1]
    #aggregatedStudyColumn <- cbind(aggregatedStudyColumn,count)
    
    aggregatedStudyColumn <- merge(aggregatedStudyColumn, count, by="binning.tmp", all.y=TRUE) 
    
    colnames(aggregatedStudyColumn) <- c(columnToStudy, "positive_response_rate", "positive_population","total_population") # changing column name of each variables in aggregatedStudyColumn dataframe
    
    # Replace NA in sum and mean to 0
    if (sum(is.na(aggregatedStudyColumn$positive_response_rate))>0) {
        aggregatedStudyColumn[which(is.na(aggregatedStudyColumn$positive_response_rate)),]$positive_response_rate <- 0
        aggregatedStudyColumn[which(is.na(aggregatedStudyColumn$positive_population)),]$positive_population <- 0
    }
    
    # Set decimal precision to 2
    aggregatedStudyColumn$positive_response_rate <- format(round(aggregatedStudyColumn$positive_response_rate, 2)) # Round Off the values
    
    # Positive population in Bin
    g1 <- ggplot(data=aggregatedStudyColumn, aes(x=aggregatedStudyColumn[,columnToStudy], y=aggregatedStudyColumn$positive_response_rate)) +
        geom_bar(stat="identity", fill="steelblue")+
        geom_text(aes(label=aggregatedStudyColumn$positive_response_rate), color="blue", vjust=-0.2, size=3.5, hjust=0.3) +
        theme_minimal() +
        labs(x = paste("Positive Response Rate in each [", columnToStudy,"] bin"), y = "Positive Response(Mean)")
    
    # Bin population
    g2 <- ggplot(data=aggregatedStudyColumn, aes(x=aggregatedStudyColumn[,columnToStudy], y=aggregatedStudyColumn$total_population)) +
        geom_bar(stat="identity", fill="steelblue")+
        geom_text(aes(label=aggregatedStudyColumn$total_population), color="darkblue", vjust=-0.2, size=3.5, hjust=0.3) +
        theme_minimal() +
        labs(x = paste("Population in each [", columnToStudy,"] bin"), y = "Total population")
    
    
    grid.arrange(g1,g2)
    return(aggregatedStudyColumn)
}

bin_group <- c(seq(min(Master_final[,"Age"]),max(Master_final[,"Age"]), length.out=10))
					
responseBinAnalysis(Master_final, columnToStudy="Age", classificationTargetColumnName="Performance.Tag", positiveEventResponse="1", 
                    negativeEventResponse="0", no.Of.Bins=10, bin_breaks=c(18,30,40,50,65))
					
input_data <- Master_final
input_data$Performance.Tag <- as.numeric(levels(input_data$Performance.Tag))[input_data$Performance.Tag]
infoTables <- create_infotables(data = input_data, y = "Performance.Tag", bins = 10, parallel = T)

data_infoTable <- infoTables
columnsToRelaceWoe <- c("Avgas.CC.Utilization.in.last.12.months", "No.of.trades.opened.in.last.12.months", "No.of.PL.trades.opened.in.last.12.months", "No.of.Inquiries.in.last.12.months..excluding.home...auto.loans.", "Outstanding.Balance", "No.of.times.30.DPD.or.worse.in.last.6.months", "Total.No.of.Trades", "No.of.PL.trades.opened.in.last.6.months", "No.of.times.90.DPD.or.worse.in.last.12.months", "No.of.times.60.DPD.or.worse.in.last.6.months", "No.of.Inquiries.in.last.6.months..excluding.home...auto.loans.", "No.of.times.30.DPD.or.worse.in.last.12.months", "No.of.trades.opened.in.last.6.months", "No.of.times.60.DPD.or.worse.in.last.12.months", "No.of.times.90.DPD.or.worse.in.last.6.months", "No.of.months.in.current.residence", "Income", "No.of.months.in.current.company", "Presence.of.open.home.loan")
replaceWithWoE <- function(input_data, data_infoTable, columnsToRelaceWoe) {
    for (columnToReplace in columnsToRelaceWoe) {
        data_infoTable$Tables[columnToReplace][columnToReplace][columnToReplace][1]
        
        x_labels <- as.list(data_infoTable$Tables[[columnToReplace]][,1])
        woe_labels <- as.list(data_infoTable$Tables[[columnToReplace]][,4])
        
        new_woe_column_name <- paste("woe_", columnToReplace, sep = "")
        input_data[,new_woe_column_name] <- NA
        for (i in length(x_labels):1){
            if (is.numeric(input_data[,columnToReplace])) { # Original input column is numeric
                aIndexLableSplitPart <- unlist(strsplit(x_labels[[i]], ","))[[2]]
                actualUpperPart <- substr(aIndexLableSplitPart, 1,nchar(aIndexLableSplitPart)-1)
                print(paste("actualUpperPart",actualUpperPart,"woe val",woe_labels[i]))
                bin_upper_limit <- as.numeric(actualUpperPart)
                woe_value_to_replace <- unlist(woe_labels[i])
                
                
                input_data[,new_woe_column_name] <- 
                    ifelse(input_data[,columnToReplace]<=bin_upper_limit, woe_value_to_replace,input_data[,new_woe_column_name])
            } else if (is.factor(input_data[,columnToReplace])) {
                level_name <- x_labels[[i]]
                woe_value_to_replace <- unlist(woe_labels[i])
                input_data[which(input_data[,columnToReplace]==level_name),new_woe_column_name]<- woe_value_to_replace
...
input_data <- replaceWithWoE(input_data, data_infoTable, columnsToRelaceWoe)

infoTableData <- infoTables$Tables

plotWoEIV <- function(infoTableData, columnToStudy) {
    variable_infotable <- infoTableData[[columnToStudy]]
    #variable_infotable$WOE <- format(round(variable_infotable$WOE, 2)) # Round Off the values
    #variable_infotable$IV <- format(round(variable_infotable$IV, 2)) # Round Off the values
    
    
    # Positive population in Bin
    # Change the color
    g1 <- ggplot(data=variable_infotable, aes(x=variable_infotable[,columnToStudy], y=variable_infotable$WOE, group=1)) +
        geom_line(color="red")+
        geom_point() + theme_minimal() + geom_label(label = sprintf("%0.2f", round(variable_infotable$WOE, digits = 2)), vjust=0.5, size=3.5, hjust=1.3) +
        labs(x = paste("WoE of each [", columnToStudy,"] bin"), y = "WoE") +
        scale_x_discrete(limits=variable_infotable[,columnToStudy])
    
    
    g2 <- ggplot(data=variable_infotable, aes(x=variable_infotable[,columnToStudy], y=variable_infotable$IV, group=1)) +
        geom_line(color="red")+
        geom_point() + theme_minimal() + geom_label(label = sprintf("%0.2f", round(variable_infotable$IV, digits = 2)), vjust=0.55, size=3.5, hjust=1.3) +
        labs(x = paste("IV of each [", columnToStudy,"] bin"), y = "IV") +
        scale_x_discrete(limits=variable_infotable[,columnToStudy])
    
    # Percent
    g3 <- ggplot(data=variable_infotable, aes(x=variable_infotable[,columnToStudy], y=variable_infotable$Percent)) +
        geom_bar(stat="identity", fill="steelblue")+
        geom_text(aes(label=sprintf("%0.2f", round(variable_infotable$Percent, digits = 2))), color="darkblue", vjust=0.2, size=3.5, hjust=0.3) +
        theme_minimal() +
        labs(x = paste("Percent Population in each [", columnToStudy,"] bin"), y = "Percent population") +
        scale_x_discrete(limits=variable_infotable[,columnToStudy])
    
    # Bin population
    g4 <- ggplot(data=variable_infotable, aes(x=variable_infotable[,columnToStudy], y=variable_infotable$N)) +
        geom_bar(stat="identity", fill="steelblue")+
        geom_text(aes(label=variable_infotable$N), color="blue", vjust=0.2, size=3.5, hjust=0.3) +
        theme_minimal() +
        labs(x = paste("N in each [", columnToStudy,"] bin"), y = "Bin Population") +
        scale_x_discrete(limits=variable_infotable[,columnToStudy])
    
    grid.arrange(g1,g2,g3,g4, ncol=1)
}

plotWoEIV(infoTableData, "Age")
